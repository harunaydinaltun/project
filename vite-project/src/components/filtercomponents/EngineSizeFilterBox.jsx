...
export const EngineSizeFilterBox = ({
  t,
  engineSizes = [],
  filters,
  onFilterChange,
  ...
  const selectedEngineSizes = filters?.engineSize || [];
      ...
      >
        <div className="overflow-hidden">
          <div className="pt-2 flex flex-col gap-1">
            {Array.isArray(engineSizes) && engineSizes.length > 0 ? (
              engineSizes.map((size) => {
                const isSelected = selectedEngineSizes.includes(size);

                return (
                  <div
                    key={size}
                    onClick={() => onFilterChange("engineSize", size)}
              ...
              <span className="text-xs text-gray-500 pl-1">
                {isDisabled ? "Önce model seçiniz" : "Motor hacmi bulunamadı"}
              </span>
            )}
          </div>
        </div>
...
export default EngineSizeFilterBox;
