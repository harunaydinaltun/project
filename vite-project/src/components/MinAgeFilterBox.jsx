...
export const MinAgeFilterBox = ({
  ...
  const selectedValue = filters?.minAge;
  ...
  return (
    <div className="bg-slate-200 ring-1 rounded-2xl mt-1.5 p-2 ring-slate-300 max-w-40 cursor-pointer transition-all hover:shadow-lg">
      <div className="flex justify-evenly" onClick={() => setIsOpen(!isOpen)}>
        <div className="pl-1 text-shadow-md">{t.minAge || "MİN. YAŞ"}</div>
        <span
      ...
      >
        <div className="overflow-hidden">
          <div className="pt-2 flex flex-col gap-1">
            {Array.isArray(minAges) && minAges.length > 0 ? (
              minAges.map((item) => {
                const isSelected = selectedValue === item;
                return (
                  <div
                    key={item}
                    onClick={() => onFilterChange("minAge", item)}
